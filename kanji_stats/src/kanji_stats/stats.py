...
class KanjiStats(object):
    ...
    def genKanjiSets(self):
        ...
        for m in self.col.models.all():
            if not isJapaneseNoteType(m['name']):
                continue

            idxs = []
            for c,n in enumerate(self.col.models.fieldNames(m)):
                name = n.lower()
                for f in conf.get("srcFields"):
                    if name == f.lower():
                        idxs.append(c)
...
def createMenu():
    ...
    name = conf.get("menu_name", "&Stats")
    js_menu = toolbar.getMenu(mw, name)
    a = QAction(mw)
    a.setText("Kanji Stats")
    a.triggered.connect(onKanjiStats)
    js_menu.addAction(a)
    remHook(ADDON_NAME+".configLoaded", createMenu)
    # Note: This is for ccbc only

addHook(ADDON_NAME+".configLoaded", createMenu)
