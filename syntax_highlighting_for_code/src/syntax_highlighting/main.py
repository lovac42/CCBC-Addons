...
mw.form.menuAddon.addAction(options_action)
...
def add_code_langs_combobox(self, func, previous_lang):
    ...
    # b.setFixedWidth(20)
    combo.setMinimumContentsLength(60)
    limited_langs = local_conf.get("limitToLangs")
    if limited_langs:
...
def addWidgets20(ed, previous_lang):
    ...
    ADDCARD=True if isinstance(ac, addcards.AddCards) else False
    height=width=None
    if not ADDCARD:
        height=width=20
    ...
    splitter.setFrameRect(rect.adjusted(0,0,-10,0))
    splitter.setMinimumWidth(120)

    if ADDCARD:
        ac.form.horizontalLayout.addWidget(splitter)
    else:
